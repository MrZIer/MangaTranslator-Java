...
public class TranslationService {
    ...
    public String translate(String text, String targetLanguage, TranslationEngine engine, Map<String, String> glossary) {
        ...
        if (redisTemplate != null) {
            try {
                ...
                log.warn("Failed to get translation from Redis cache: {}", e.getMessage());
            }
        }
        ...
        if (redisTemplate != null) {
            try {
                ...
                log.warn("Failed to cache translation to Redis: {}", e.getMessage());
            }
        }
    ...
    private String translateWithZhipu(String text, String targetLanguage) {
        ...
        try {
            log.info("Calling Zhipu API with model: {}", zhipuModel);
            ...
            if (response != null && response.containsKey("choices")) {
                ...
                if (!choices.isEmpty()) {
                    ...
                    log.info("Zhipu translation successful, result length: {}", translated.length());
                    return translated;
                }
            }

            log.error("Invalid response from Zhipu API: {}", response);
            throw new RuntimeException("Invalid response from Zhipu API");

        } catch (Exception e) {
            log.error("Zhipu translation failed for text: '{}', error: {}", 
